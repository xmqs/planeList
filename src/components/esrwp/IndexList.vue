<template>
    ...
    <mt-index-list v-if="lists"  :show-indicator="true">
      <mt-index-section v-for="(ele,index) in arealist" :index="ele.firstKey" :key="index">
        <p @click="bus(element.name + element.pinyin)" v-for="(element,index) in ele.cities" :key="index">
          <mt-cell :class="{'class-city':active == element.name}" class="sp" :title="element.name +'  '+ element.pinyin"></mt-cell>
        </p>
      </mt-index-section>
    </mt-index-list>
	</div>
